  ...
  return (
    <div>
        ...
        <div className="w-full h-full flex justify-center items-center">
          ...
          <div className="absolute top-2 right-11 group-hover:right-1 p-2 flex flex-col items-center justify-center gap-y-2 opacity-0 group-hover:opacity-100 transition-all duration-300">
            <button onClick={() => addToCart(product, id)}>
              <div className="flex justify-center items-center text-white w-8 h-8 bg-purple-800">
                <BsPlus className="text-3xl" />
              </div>
            </button>
            <Link
              to={`/product/${id}`}
              className="w-8 h-8 bg-white flex justify-center items-center rounded-sm text-pink-700 drop-shadow-xl border border-[#e9e9e9]"
            >
              <BsEyeFill />
            </Link>
          </div>
      ...
      <div>
        ...
        <div className="font-semibold text-pink-600 text-lg">${price}</div>
      </div>
    </div>
  );
};
